#!/usr/bin/env python
"""
Clear candidates from SQLite with correct table names
"""
import sqlite3
import os

def clear_sqlite_correct():
    """Clear candidates from SQLite using correct table names"""
    ...
    try:
        ...
        if candidate_count > 0:
            # Show candidates
            cursor.execute("SELECT work_id, name, email FROM candidates ORDER BY work_id")
        ...
        if candidate_count > 0 or offer_letter_count > 0:
            ...
            cursor.execute("DELETE FROM sqlite_sequence WHERE name='offers_offerletter'")
