...
from offers.models import Template, Candidate
import os

User = get_user_model()

class Command(BaseCommand):
    help = 'Update Neon database with templates and users'

    def handle(self, *args, **options):
        self.stdout.write('🔄 Updating Neon database...')
        
        # Clear existing templates in Neon
        self.stdout.write('🗑️  Clearing existing templates from Neon...')
        Template.objects.using('neon').all().delete()
        ...
        for mapping in template_mappings:
            ...
            if os.path.exists(filepath):
                try:
                    template = Template.objects.using('neon').create(
                        name=mapping['name'],
                        role=mapping['role'],
                    ...
                    created_count += 1
                    self.stdout.write(
                        self.style.SUCCESS(f'✅ Created in Neon: {template.name} ({template.get_role_display()})')
                    )
                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(f'❌ Error creating {mapping["name"]} in Neon: {str(e)}')
                    )
            else:
                self.stdout.write(
        ...
        self.stdout.write(f'\n📊 Templates created in Neon: {created_count}')
        
        # Create users in Neon (if auth is using Neon)
        self.stdout.write('\n👥 Creating users in Neon...')
        ...
        for user_data in users_data:
            ...
            if User.objects.using('neon').filter(username=username).exists():
                user = User.objects.using('neon').get(username=username)
                ...
                self.stdout.write(
                    self.style.WARNING(f'⚠️  Updated existing user in Neon: {username}')
                )
            else:
                # Create new user in Neon
                user = User(username=username, email=user_data['email'], first_name=user_data['first_name'], last_name=user_data['last_name'], is_staff=user_data['is_staff'], is_superuser=user_data['is_superuser'])
                user.set_password('Temp@123456')  # Default password
                user.save(using='neon')
                self.stdout.write(
                    self.style.SUCCESS(f'✅ Created user in Neon: {username} (Password: Temp@123456)')
                )
                created_users += 1
        
        self.stdout.write(f'\n👥 Users created in Neon: {created_users}')
        
        # Display summary
        self.stdout.write('\n' + '='*50)
        self.stdout.write(self.style.SUCCESS('🎉 Neon database update completed!'))
        self.stdout.write('\n📋 Summary:')
        self.stdout.write(f'   • Templates: {created_count} created in Neon')
        self.stdout.write(f'   • Users: {created_users} created in Neon')
